...
//    [[UIApplication sharedApplication] cancelAllLocalNotifications]; //清理系统通知
    ...
    if (![ICNetworkManager defaultManager].token || [[ICNetworkManager defaultManager].token isEqualToString:@""]) {
        UIViewController *controller = [[NSBundle mainBundle] loadNibNamed:@"ICLoginViewController2" owner:nil options:nil].firstObject;
        [self.window setRootViewController:controller];
    } else {
        UIViewController *controller = [[ICGateViewController2 alloc] initWithCollectionViewLayout:[[UICollectionViewFlowLayout alloc] init]];
